...
Ported from extract_coderabbit_prompts.py with improvements for structured
data extraction.
"""
...
class ExtractedContent:
    ...
    def __init__(
        self, content_type: str, content: str, metadata: dict[str, Any] | None = None
    ):
...
def parse_file_reference_from_body(body: str) -> str | None:
    ...
    file_in_text_pattern = r"In ([^\s]+\.py) around lines? (\d+(?:-\d+)?|\d+ to \d+)"
...
def extract_committable_suggestions(
    body: str, user_login: str
