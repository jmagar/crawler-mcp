...
def _detect_target(target: str) -> Literal[
    "website", "github_pr", "repository", "directory"
]:
        ...
        for p in root.rglob(pat):
            ...
            pages.append(
                PageContent(
                    ...
                    title=p.name,
                    content=content,
                    markdown=content if p.suffix.lower() in {".md", ".markdown"} else None,
                    html=None,
                    links=[],
                    images=[],
                    metadata={"path": str(p)},
...
async def _crawl_repository(
    ...
    try:
        ...
        pages = await _crawl_directory(
            ctx, tmpdir, include=include, exclude=exclude, max_files=max_files, file_size_limit_bytes=file_size_limit_bytes
        )
        return pages, tmpdir
    except Exception as e:
    ...
    async def scrape(
        ...
        try:
            ...
            try:
                ...
                await ctx.info(f"Strategy returned success: {getattr(resp, 'success', 'unknown')}")
            except Exception as e:
                await ctx.info(f"Strategy.crawl threw exception: {type(e).__name__}: {e}")
            ...
            if not clean and (os.getenv("OPTIMIZED_CRAWLER_SCRAPE_RETRY_JS", "true").lower() == "true"):
                tried_js_retry = True
            ...
            content_preview = (clean or getattr(resp, "extracted_content", "") or "").strip()[:500]
            ...
            if network_success and not content_extracted:
                await ctx.info(f"Network request succeeded but no content extracted from {url}")
    ...
    async def crawl(
        ...
        try:
            ...
            if kind in ("website", "github_pr"):
                ...
                try:
                    failed_ct = int(resp.response_headers.get("X-Failed-URLs-Count", "0"))
                except Exception:
                    failed_ct = 0
                pages_failed = failed_ct
                duration_s = float(getattr(getattr(resp, "metadata", {}), "get", lambda *_: 0)("duration_seconds", 0)) or 0.0
                ...
                paths = output.get_crawl_output_paths(f"https://{domain_for_index}", session_id)
                output.save_crawl_outputs(
                    domain=domain_for_index,
                    html=None,
                    pages=pages,
